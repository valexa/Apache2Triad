...
ExecWait '"$INSTDIR\bin\instsrv.exe" Apache2 $INSTDIR\bin\apache.exe'
...
WriteRegExpandStr HKLM "SYSTEM\CurrentControlSet\Services\Apache2" "ImagePath" '"$INSTDIR\bin\apache.exe"$R5 -n Apache2 $R6'
...
ExecWait '"$INSTDIR\bin\instsrv.exe" Apache2SSL $INSTDIR\bin\apache.exe'
...
WriteRegExpandStr HKLM "SYSTEM\CurrentControlSet\Services\Apache2SSL" "ImagePath" '"$INSTDIR\bin\apache.exe"$R5 -D SSL -n Apache2SSL $R6'
...
WriteRegStr HKLM "SOFTWARE\ODBC\ODBCINST.INI\MyODBC" "Driver" "$INSTDIR\mysql\bin\myodbc3.dll"
WriteRegStr HKLM "SOFTWARE\ODBC\ODBCINST.INI\MyODBC" "Setup" "$INSTDIR\mysql\bin\myodbc3.dll"
...
WriteRegStr HKLM "SOFTWARE\ODBC\ODBCINST.INI\MyODBC" "DriverODBCVer" "03.51"
...
WriteRegStr HKLM "SOFTWARE\ODBC\ODBC.INI\MySQL" "Driver" "$INSTDIR\mysql\bin\myodbc3.dll"
...
Section "Required"
  ...
  CreateShortCut "$STARTMENU\${NAME}\WinmysqlAdmin.lnk" "$INSTDIR\mysql\bin\winmysqladmin.exe" "" ""
...
Section "Mysql"
  ...
  CreateShortCut "$STARTMENU\${NAME}\MySqlManager.lnk" "$INSTDIR\mysql\bin\mysqlmanager.exe" "" ""
SectionEnd
Section "PostgreSQL"
...
ExecWait "killproc /K apache.exe"
ExecWait "killproc /K winmysqladmin.exe"
